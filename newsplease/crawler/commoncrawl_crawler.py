...
class CommonCrawlCrawler:
    ...
    def __run(self):
        ...
        self.__cc_news_crawl_names = self.__get_remote_index()
        self.__logger.info('found %i files at commoncrawl.org', len(self.__cc_news_crawl_names))

        # iterate the list of crawl_names, and for each: download and process it
        for name in self.__cc_news_crawl_names:
    ...
    def crawl_from_commoncrawl(self, callback_on_article_extracted, valid_hosts=None, start_date=None, end_date=None,
                               strict_date=True, reuse_previously_downloaded_files=True, local_download_dir_warc=None,
                               continue_after_error=True, show_download_progress=False, log_level=logging.ERROR):
        """
